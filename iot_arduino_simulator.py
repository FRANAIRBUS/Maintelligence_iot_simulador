...
Emula el flujo de un ESP/Arduino que:
- hace bootstrap contra iotDeviceBootstrap
- guarda deviceSecret
- sincroniza telemetría firmando con HMAC SHA256
- recibe desiredState desde la app
- opcionalmente aplica desiredState sobre el estado local
...
Uso:
  python iot_arduino_simulator.py
"""
...
class App(tk.Tk):
    ...
    def _build_connection_tab(self):
        ...
        hint = (
            "Flujo normal: 1) desde la app genera provisioning token, 2) pega organizationId/deviceKey/bootstrapToken/bootstrapUrl, "
            "3) pulsa Bootstrap, 4) pulsa Sync ahora o activa Auto-sync."
        )
    ...
    def save_current_config(self):
        self._save_from_ui()
        save_config(self.config_data)
        self.log(f"Config guardada en {CONFIG_PATH.resolve()}")
    ...
    def import_config(self):
        target = filedialog.askopenfilename(filetypes=[("JSON", "*.json")])
        if not target:
            return
